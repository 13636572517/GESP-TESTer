          ...
          <el-card>
            ...
            <div v-for="(answer, i) in result.answers" :key="answer.id" style="padding: 12px 0; border-bottom: 1px solid #E5E7EB">
              ...
              <div v-if="!answer.is_correct" style="font-size: 13px; margin-top: 4px">
                <span style="color: #D92916">你的答案: {{ answer.user_answer || '未作答' }}</span>
                <span style="color: #00A60E; margin-left: 12px">正确答案: {{ answer.question?.answer }}</span>
              </div>
              <div v-if="answer.question?.explanation" style="font-size: 13px; color: #6B7280; margin-top: 4px">
